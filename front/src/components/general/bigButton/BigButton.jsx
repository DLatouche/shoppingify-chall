...
const BigButton = ({ variant, onClick, children, className, disabled }) => {
  ...
  const renderRippleSpan = () => {
    ...
    if (spanArray && spanArray.length > 0) {
      return spanArray.map((key, index) => {
        return (
          <span
            className="bigButton__ripple__content"
            key={"spanCount_" + index}
            style={{ ...combinedSpanStyles[key] }}
          ></span>
        )
      })
    } else {
  ...
  let classNameCalculate = "bigButton bigButton--" + variant + " " + className
  if (disabled) classNameCalculate += " bigButton--disabled"
  return (
    <button className={classNameCalculate} onClick={onClick} disabled={disabled}>
      {children}
