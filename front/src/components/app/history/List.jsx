...
const List = ({ list, className, onClick }) => {
  let date = list.createdAt.toLocaleDateString("en-US", { weekday: "short" })
  let strDate =
    date + " " + list.createdAt.getDate() + "." + (list.createdAt.getMonth() + 1) + "." + list.createdAt.getFullYear()
  return (
    <div className={"list " + className}>
      <p className="list__name">{list.name}</p>
...
const ListStore = ({ list, className, onClick }) => {
  return <List list={list} className={className} onClick={onClick} />
}
