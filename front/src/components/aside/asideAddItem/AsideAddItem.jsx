...
const AsideAddItem = ({ className, listCategories, setAside, currentList, addToCurrentList, addItem, addCategory }) => {
  ...
  const saveItem = async () => {
    ...
    if (result.status === "FAILED")
      console.log(
        "%cAsodeAddItems.jsx -> 24 ERROR: result.message",
  ...
  return (
    <div className={className + " asideAddItem"}>
      ...
      />
      <Select
        className="asideAddItem__select"
        onChange={onSelectChange}
        value={item.category.id}
        list={listCategories}
      />
      <div className="asideAddItem__actions">
        <BigButton className="asideAddItem__actions__button" variant="transparent" onClick={onCancel}>
          cancel
        </BigButton>
        <BigButton className="asideAddItem__actions__button" variant="primary" onClick={saveItem}>
          save
        </BigButton>
      </div>
    </div>
