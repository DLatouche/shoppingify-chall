...
const ItemList = ({ item, removeFromList, listEditing, update }) => {
  ...
  if (stateItem.isChecked) containerClass += " itemList--checked"
  return (
    <div className={containerClass}>
      {stateItem.isChecked ? (
        <CheckBoxOutlinedIcon onClick={onCheck} className="itemList__checkbox" />
      ) : (
        <CheckBoxOutlineBlankOutlinedIcon onClick={onCheck} className="itemList__checkbox" />
      )}
      <p onClick={onClickName} className="itemList__name">
        {stateItem.name}
      </p>
      <div className={!isOpened ? "ItemList__button" : "ItemList__button ItemList__button--editing"}>
        ...
        <p onClick={toggleOpen} className="ItemList__button__quantity">
          {stateItem.quantity}
          <em className="ItemList__button__quantity_pcs">pcs</em>
        </p>
...
export const ItemListStore = ({ item, removeFromList, editing, update, listState }) => {
  return <ItemList item={item} removeFromList={removeFromList} listEditing={editing} update={update} />
}
